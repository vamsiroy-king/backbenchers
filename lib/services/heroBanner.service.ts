        ...
        try {
            const { data, error } = await supabase
                ...
                .insert({
                    ...
                    merchant_id: banner.merchantId || null, // Ensure null if empty string/undefined to avoid UUID error
                    is_auto_generated: !!banner.isAutoGenerated, // Strict boolean
                    banner_type: banner.bannerType || 'promotion',
                    coverage_type: banner.coverageType || 'pan_india',
                    city_ids: (banner.cityIds && banner.cityIds.length > 0) ? banner.cityIds : null, // Send null if empty
                    start_date: banner.startDate || new Date().toISOString(),
                    end_date: banner.endDate || null,
                    position: banner.position || 0,
                    is_active: banner.isActive ?? true,
                    event_date: banner.eventDate || null,
                    event_location: banner.eventLocation || null,
                    organizer_name: banner.organizerName || null,
                    organizer_contact: banner.organizerContact || null,
                })
                .select()
                .single();
