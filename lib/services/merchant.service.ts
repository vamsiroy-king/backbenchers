...
export const merchantService = {
    ...
    async approve(id: string): Promise<ApiResponse<Merchant>> {
        try {
            ...
            // 6. Auto-generate Hero Banner for new store (shows for 14 days)
            try {
                // Get merchant's first offer for banner details
                const { data: firstOffer } = await supabase
                    .from('offers')
                    .select('title, discount_value, type, description')
                    .eq('merchant_id', id)
                    .eq('status', 'active')
                ...
                // Create auto-generated banner
                const bannerEndDate = new Date();
                bannerEndDate.setDate(bannerEndDate.getDate() + 14); // Show for 14 days

                const discountText = firstOffer
                    ? (firstOffer.type === 'flat'
                        ? `₹${firstOffer.discount_value} OFF`
                        : `${firstOffer.discount_value}% OFF`)
                    : 'Exclusive Deals';

                await heroBannerService.create({
                    title: `${pendingData.business_name} is now on BackBenchers!`,
                    subtitle: firstOffer?.title || `Get ${discountText} - Student Exclusive`,
                    ctaText: 'View Offers →',
                    ctaLink: `/store/${id}`,
                    backgroundGradient: 'from-green-500 via-green-600 to-green-800',
                    imageUrl: pendingData.cover_photo_url || null,
                    logoUrl: pendingData.logo_url || undefined,
                    bannerType: 'new_store',
                    coverageType: pendingData.city ? 'city_specific' : 'pan_india',
                    cityIds: null, // Will show everywhere
                    startDate: new Date().toISOString(),
                    endDate: bannerEndDate.toISOString(),
                    position: 999, // Low priority, admin can reorder
                    isActive: true,
                    merchantId: id,
                    isAutoGenerated: true,
                    organizerName: pendingData.business_name
                });
